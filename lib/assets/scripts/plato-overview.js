$(function($){
  ...
  function drawOverviewCharts(reports) {
    ...
    reports.forEach(function(report){
      const aggregate = report.complexity.methodAggregate;
      // @todo shouldn't need this, 'auto [num]' doesn't seem to work : https://github.com/oesmith/morris.js/issues/201
      sloc.ymax = Math.max(sloc.ymax, aggregate.sloc.physical);
      bugs.ymax = Math.max(bugs.ymax, aggregate.halstead.bugs.toFixed(2));


      sloc.data.push({
        value : aggregate.sloc.physical,
        label : report.info.fileShort
      });
      bugs.data.push({
        value : aggregate.halstead.bugs.toFixed(2),
        label : report.info.fileShort
      });
